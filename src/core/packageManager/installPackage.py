                ...
                if self.isInstallable():
                    ...
                    if not self.parser.keyExists(self.scriptFile, "unzip"):
                        if self.valid_exit_code():
                            self.parser.addNewPackage(self.packageName, {"version": self.scriptFile['version'],
                            ...
                            helpers.errorMessage(
                                "{0} returned was not installed successfully.".format(self.packageName))
                            if helpers.is_verbose():
                                helpers.errorMessage(
                                    '{} returned {} exit code.'.format(self.packageName, self.exit_code))
                            return False
                    else:
                        helpers.successMessage("Successfully installed " + self.packageName)
                        self.printNotesFromParser()
    ...
    def download(self):
        ...
        if not file.Manager().fileExists(file_path.format('')):
            ...
            if self.parser.keyExists(self.scriptFile, 'arches'):
                ...
                return True
            helpers.infoMessage("Downloading {} from: {}".format(self.packageName, download_url))
            httpClass.download(download_url, download_path, loadJson["fileType"])
    ...
    def unzipPackage(self):
        ...
        for i in extensions:
            if i == self.scriptFile["fileType"]:
                bit_64 = self.scriptFile['extractFolder']['64bit'] if self.parser.keyExists(self.scriptFile,
                                                                                            'extractFolder') else None
                bit_32 = self.scriptFile['extractFolder']['32bit'] if self.parser.keyExists(self.scriptFile,
                                                                                            'extractFolder') else None
                extractFolder = None
            ...
            self.parser.addNewPackage(self.packageName,
                                      {"version": self.scriptFile['version'], 'dependencies': self.dependencies})
            ...
            if e.winerror == 193:
                ...
                if self.arches:
                    call_exe64 = subprocess.Popen(
                        '"{0}" {1}'.format(self.install_path64, self.scriptFile["silentArgs"]), shell=True)
                    call_exe64.communicate()[0]
