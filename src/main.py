...
test_package.add_argument("testpackage", type=str,
...
update.add_argument("update", action="store_true",help="Update available package list on your system.")

#Doctor command
doctor = subparser.add_parser('doctor')
doctor.add_argument("doctor", action="store_true",help="Fixes common problems.")
...
if hasattr(arg, 'remove') or arg.R:
    ...
    PackageManager.Manager(package_name, skip_hash,
                           force, agreement, True).removePackage()

if hasattr(arg, 'upgrade'):
    package_name = arg.upgrade
    PackageManager.Manager(package_name, skip_hash,
                           force, agreement).upgradePackage()
